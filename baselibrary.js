...
let IntersectionFrame = (props) => ({
	draw: (p, grid) => {
		...
		let side = props.side
		if (side == 'left') {
			let x = grid.verso.width.px
			p.push()
			p.translate(x, 0)
		}

		else {
			let x = grid.recto.width.px
			p.push()
			p.translate(x * -1, 0)
		}
...
let imageframe = props => {
	...
	return {
		draw: (p, prop) => {
			...
			if (pop) {
				p.image(image, 0, 0, w.px, h)
				p.pop()
			}
			else {
				p.image(image, x.px, y.px, w.px, h)
			}
			p.opacity(1)
		}
	}
...
export let drawSignature = (p, spreads, sheets, centerx, y, spreadNum) => {
	...
	drawVerso(p, spreads, sheets, centerx, y, spreadNum, true)
	drawRecto(p, spreads, sheets, centerx, y, spreadNum, true)
	p.stroke(180)
...
let drawRecto = (p, spreads, sheets, centerx, y, spreadNum, draw_behind = false) => {
	...
	if (draw_behind) {
		...
		if (after.length > 0) drawRecto(p, spreads, sheets, centerx, y, after[0])
	}
	...
	if (recto_dimensions.offset) {
		if (recto_dimensions.offset.vertical) y += recto_dimensions.offset.vertical.px
	}
...
let drawVerso = (p, spreads, sheets, centerx, y, spreadNum, draw_behind = false) => {
	...
	let vw = verso_dimensions.width.px
	let vy = y
	let vx = centerx - vw
	if (verso_dimensions.offset) {
		if (verso_dimensions.offset.vertical) vy += verso_dimensions.offset.vertical.px
	}
	...
	if (draw_behind) {
		...
		if (before.length > 0) drawVerso(p, spreads, sheets, centerx, y, before[before.length - 1],)
	}
...
let verso_image = (p, spreads, sheets, pageNum,) => {
	...
	if (dimensions.recto.height.px > h.px) h = dimensions.recto.height

	let buffer = p.createGraphics(w.px, h.px)
	if (!printing) buffer.background(dimensions.verso.color)
	else buffer.background('white')

	spreads[pageNum].draw(buffer, dimensions)
	if (pageNum == 0) { buffer.background('#eee') }
	let page_width = dimensions.verso.width.px
	let img = buffer.get(0, 0, page_width, dimensions.verso.height.px)
...
let recto_image = (p, spreads, sheets, pageNum,) => {
	...
	let h = dimensions.verso.height
	if (h == undefined || dimensions.recto.height.px > h.px) h = dimensions.recto.height
	...
	let img = buffer.get(dimensions.verso.width.px, 0,
		dimensions.recto.width.px, dimensions.recto.height.px)
	return img
}
...
export let drawAccordionBook = (p, signatures) => {
	...
	let spreadsgrouped = arr => arr.reduce((acc, sig, i) => {
		...
		let dim = sig.sheets[0]
		acc[i].push(dim.width.px / 2)
		acc.push([])
		acc[i + 1].push(dim.width.px / 2)
		return acc
	}, [[0]])
...
function draw_line(p, text, x, y, length, state, hooks) {
	...
	words.forEach((word, index) => {
		...
		if (tag) {
			...
			if (tag.negative){
				line_state.negative_marker = -1*tag.negative.px
			}

			if (tag.positive){
				if (line_state.horizontal_pos != 0) line_state.horizontal_pos += tag.positive.px
			}

			if (tag.positive_y){
				line_state.positive_y_marker = tag.positive_y.px
			}
		...
		p.text(word, x.px + line_state.horizontal_pos + negative_marker , y.px + positive_y_marker)
		p.fill(_fill)
...
let image = (props) => ({
	...
	draw: (p) => {
		...
		let ratio = w.px / image.width
		let h = image.height * ratio
		p.image(img, props.x.px, props.y.px, w, h)
	}
})
